...
| Path | What |
|---|---|
| `engine/src/pdf_unlock_engine/` | All the logic. No CLI dependencies — future web/api/mcp surfaces import this directly. |
...
- `unlock.py` — the actual decryption. Deliberately the smallest module: pikepdf
  wraps qpdf, so it is "open it, save it without encryption".
...
uv run pytest -q
uv run ruff check .
uv tool install .            # global `pdf-unlock`
```
...
- The `make_pdf` fixture generates encrypted PDFs on the spot. Nothing real is
  ever committed — `.gitignore` blocks `*.pdf`.
